...
double VolumeGravityConstrainedDistance::operator()(const Datum& ai, const Datum& aj,
                                     const Datum& bi, const Datum& bj)
{
  ...
  const double volume_score_i = ai(3) < bi(3) ? ai(3) / bi(3) : bi(3) / ai(3);
  const double volume_score_j = aj(3) < bj(3) ? aj(3) / bj(3) : bj(3) / aj(3);
  ...
  return (c_xy<params_.epsilon && c_z<params_.epsilon && 
    ...
    * volume_score_i * volume_score_j, 1.0/3.0) : 0;
}
