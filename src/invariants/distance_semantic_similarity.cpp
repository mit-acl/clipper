...
double DistanceSemanticSimilarity::single_similarity(const Datum& ai, const Datum& bi)
{
  ...
  const Datum ai_feat = ai.segment(params_.point_dim, 768);
  const Datum bi_feat = bi.segment(params_.point_dim, 768);

  // const float cosine_score  = (ai_feat.transpose() * bi_feat)(0) / (ai_feat.norm() * bi_feat.norm());
  const float cosine_score  = (ai_feat.transpose() * bi_feat)(0);
  ...
  // }
  // else {
  //     return (cosine_score - params_.cosine_min) / (params_.cosine_max - params_.cosine_min);
  // }
  return 0.0;
