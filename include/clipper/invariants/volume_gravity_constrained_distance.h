/**
 * @file volume_gravity_constrained_distance.h
...
namespace invariants {
  ...
  /**
   * @brief      Specialization of PairwiseInvariant to Volume Gravity Constrained distance in
   *             the real numbers using the 2-norm as the invariant.
   */
  class VolumeGravityConstrainedDistance : public PairwiseInvariant
  {
  public:
    struct Params
    {
      ...
      double epsilon_volume = 0.2;
    };
  public:
    VolumeGravityConstrainedDistance(const Params& params)
    : params_(params) {}
    ~VolumeGravityConstrainedDistance() = default;
  ...
  using VolumeGravityConstrainedDistancePtr = std::shared_ptr<VolumeGravityConstrainedDistance>;
